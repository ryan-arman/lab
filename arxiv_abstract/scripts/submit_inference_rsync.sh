...
# Parse arguments
INPUT_FILE="${1:-data/arxiv_summarization_test_instruct.jsonl}"
...
# For relative paths starting with .., resolve from current directory
# For other relative paths, resolve from LOCAL_BASE_DIR
if [[ "${INPUT_FILE}" != /* ]]; then
...
ssh "${CLUSTER_HOST}" "mkdir -p ${CLUSTER_BASE_DIR} && mkdir -p ${CLUSTER_BASE_DIR}/logs && mkdir -p ${CLUSTER_BASE_DIR}/data && mkdir -p ${CLUSTER_BASE_DIR}/configs"
...
echo "  ssh ${CLUSTER_HOST} 'tail -f ${CLUSTER_BASE_DIR}/logs/arxiv_abstract_inference_qwen3_4b_*.log'"
echo ""
